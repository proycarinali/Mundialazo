...
# ... (Todo tu código superior se mantiene igual)

def obtener_datos_partido_especifico(partido_id: str = None):
    resultado = {"detalles": {"partido": "Buscando partido reciente..."}, "jugadores": []}
    ...
    try:
        # 1. Buscar partido (si viene ID lo filtramos, sino traemos el último)
        query_params = {"limit": 1, "select": "*"}
        if partido_id:
            query_params["id_partido"] = f"eq.{partido_id}"
        else:
            query_params["order"] = "fecha_partido.desc"

        partidos = supabase_get("partidos", query_params)
...
@app.get("/api/trivias")
async def obtener_trivias(partido_id: str = None, nombre_partido: str = None):
    if not openai_client:
        return {"error": "OPENAI_API_KEY no configurada"}

    # Obtenemos los datos filtrados si poseemos id, sino intentamos por defecto
    datos = obtener_datos_partido_especifico(partido_id)
    info_jugadores = datos.get("jugadores", [])[:15]

    # Ajuste dinámico del prompt si no tenemos datos en Supabase para ese partido
    if not info_jugadores:
        contexto_partido = nombre_partido if nombre_partido else "el último partido importante de esta liga"
        prompt_contenido = (
            f"Eres un experto en fútbol. Basándote en tus conocimientos del partido '{contexto_partido}', "
            f"crea 10 preguntas de trivia variadas y sumamente desafiantes. "
            f"Incluye detalles sobre goleadores, sustituciones clave, estadísticas o hitos de los jugadores. "
            f"IMPORTANTE: todas las respuestas deben ser 100% verídicas de la realidad histórica de ese juego. "
            f"Formato de salida SOLO JSON sin texto adicional ni backticks: "
            f"{{\"preguntas\": [{{\"\pregunta\": \"...\", \"opciones\": [\"A\",\"B\",\"C\"], \"correcta\": \"...\"}}]}}"
        )
    else:
        prompt_contenido = (
            f"Crea 10 preguntas de trivia basándote estrictamente en estos jugadores y partido real de la base de datos: "
